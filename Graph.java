...
public class Graph {
    ...
    private void createAndConnectEdges() {
        // TODO
        Graph graph = new Graph(this.stops, this.lines);
        String transportType = "Walking";
        int time;
        int distance;
        for(Stop stop: graph.getStops()){
            for(Stop stop2: graph.getStops()){
                
            }
            Stop stop1 = stop;
            distance = stop1.distanceTo(stop2);
            //time = distance * transportType.getType();
            Edge edge1 = new Edge(stop1, stop2, transportType, null, time, distance);
            this.edges.add(edge1);
            stop1.addForwardEdge(edge1);
            stop2.addBackwardEdge(edge1);
            
            
            //could run through every edge and find when stop1.forward(edge) is equal to stop2.backwards(edge)
        }
    }
    ...
    public void computeNeighbours(){
        ...
        //assumes no walking edges at point
