...
public class Board {
  ...
  private boolean isRowFull(int y) {
     ...
     return rangeX().allMatch(x -> read(x, y).isPresent());
   }
  ...
  public void clearFullRows() {
	  //if bottom row .isRowFull() then .clearRow()
	  //return rangeX().allMatch(x -> {
		  //this.read(x, y).filter(cell -> !cell.equals(Color.EMPTY)).isPresent());
	  	  //Board.clearRow();
    //hint: use clearRow
